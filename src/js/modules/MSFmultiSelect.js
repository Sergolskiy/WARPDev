export default class MSFmultiSelect {
  constructor(select, settings = {}) {
    ...
    this.defultSettings = {
      ...
      height: 30,
      appendTo: 'body',
      className: 'msf_multiselect_textarea'
    };
  ...
  create() {
    ...
    var label = document.createElement('label');
    label.className = 'see-all';
    var li = document.createElement('LI');
    var input = document.createElement('input');
    input.type = 'checkbox';
    input.disabled = (this.settings.readOnly) ? true : false;
    input.addEventListener('click', function () {
      this.closest('.msf_multiselect').classList.add('show-all');
      this.parentElement.parentElement.style.display = 'none';
    });
    var caption = document.createTextNode('See all ');
    li.appendChild(input);
    li.appendChild(caption);
    label.appendChild(li);
    ul.appendChild(label);
    ...
    document.addEventListener('click', function (event) {
      ...
      if (!isClickInside) {
        ...
        self.list.closest('.multiselect-wrap').classList.remove('multiselect-open');
      }
    });
    this.logger.addEventListener('click', function () {
      if (self.list.style.display == 'none') {
        ...
        this.closest('.multiselect-wrap').classList.add('multiselect-open');
      } else {
        self.list.style.display = 'none';
        this.classList.remove('open');
        this.closest('.multiselect-wrap').classList.remove('multiselect-open');
      }
    });
  }
  ...
  getSource() {
    ...
    for (var i = 0; i < this.select.children.length; i++) {
      data.push({
        ...
        caption: this.select.children[i].innerText,
        selected: this.select.children[i].selected
      });
    }
    return data;
  }
